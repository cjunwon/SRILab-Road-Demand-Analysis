...
def edge_draw(G, pos, measures, measure_name):
    ...
    plt.savefig('EdgeCentrality_degree_DistWt_Plot.eps', format='eps')
    plt.show()
...
NetworkEdgeWt_pre = distance_array[:,0]
...
EdgeCentrality_degree_DistWt = nx.edge_betweenness_centrality(G, weight = 'weight')
...
# EdgeCentrality_degree_DistWt = nx.edge_betweenness_centrality(G, weight = 'weights')

# subax5 = plt.figure(num=5, figsize=(10,6))
# edge_draw(G, d, EdgeCentrality_degree_DistWt, 'edge betweenness centrality with time taken weight')

    


# create a binary pickle file 
f = open("EdgeCentrality_degree_DistWt_save.pkl","wb")

# write the python object (dict) to pickle file
pickle.dump(EdgeCentrality_degree_DistWt,f)
